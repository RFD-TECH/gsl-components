...
The watermark comes from a bundled set of five brand marks: `rings`, `lattice`, `quatrefoil`,
`bird`, `crossed`. Omit `mark` and one is chosen by hashing `label` (FNV-1a), so the same label
always draws the same mark and a row of differently-labelled cards spreads across the set without
anyone picking. Pass an id to fix a specific one, a node to supply your own artwork, or `false` to
draw none.

```tsx
<MetricCard variant="soft" mark="quatrefoil" label="Compliance Rate" value="60%" />
